# Documentação de APIs REST com OpenAPI 3.0
...
Controller 1: AutoresController
| Método | Rota              | O que faz              | Parâmetros   |
| ------ | ----------------- | ---------------------- | ------------ |
| GET    | /api/autores      | Lista todos os autores | Sem parâmetro|
| GET    | /api/autores/{id} | Busca autor por ID     | id (int)     |
| POST   | /api/autores      | Cria um novo autor     | objeto Autor |
| PUT    | /api/autores/{id} | Atualiza autor         | id + Autor   |
| DELETE | /api/autores/{id} | Remove autor           | id           |

Controller 2: emprestimosController
| Método | Rota                           | O que faz          | Parâmetros        |
| ------ | ------------------------------ | ------------------ | ----------------- |
| GET    | /api/emprestimos               | Lista empréstimos  | Sem parâmetro     |
| POST   | /api/emprestimos               | Realiza empréstimo | objeto Emprestimo |
| PATCH  | /api/emprestimos/{id}/devolver | Marca devolução    | id                |

Controller 3: LivrosController
| Método | Rota             | O que faz          | Parâmetros         |
| ------ | ---------------- | ------------------ | ------------------ |
| GET    | /api/livros      | Lista livros       | pode ter paginação |
| GET    | /api/livros/{id} | Busca livro por ID | id                 |
| POST   | /api/livros      | Cria livro         | objeto Livro       |
| PUT    | /api/livros/{id} | Atualiza livro     | id + Livro         |
...
Resposta: Autor, Emprestimo, Livro e RespostaPaginada  como modelo auxiliar.

- Quais campos são obrigatórios (`[Required]`)?

Resposta: Na entidade Autor é obrigatorio apenas o nome do autor, 
          na entidade Emprestimo o LivroId e DataEmprestimo,
          na entidade Livro o Titulo e AutorId.

- Quais validações existem (`[StringLength]`, `[Range]`)?

Resposta:[StringLength] → limite de caracteres
         [Range] → valores numéricos válidos
        
- Qual a relação entre as entidades? (Autor → Livro → Empréstimo)

Resposta: Autor (1)  (N) Livro --> Um autor pode ter varios livros mais um livro pode ter apenas 1 autor
          Livro (1)  (N) Empréstimo --> Um livro pode ter  varios Emprestimo mais um emprestimo esta vinculado a 1 livro especifico
...
Resposta: 409  conflito de estado (ex: livro já emprestado)

- O que acontece ao enviar um POST sem os campos obrigatórios?

Resposta: 400 erro de validação (dados inválidos)
...
Resposta: Sim, estão todos agupados

- Clique em um endpoint e depois em "Try it out" — funciona?

Resposta: Sim, estão funcionando

- Os modelos de dados aparecem na seção "Schemas" no final?

Resposta: Sim, os modelos de dados aparecem na seção Schemas do Swagger, contendo suas propriedades e tipos automaticamente gerados a partir das classes da aplicação.

- O que está faltando na documentação? (descrições, exemplos, etc.)

Resposta: Esta faltando descrições nos endpoints, explicação dos parâmetros, exemplos de requisição e resposta, além de não documentar adequadamente os códigos HTTP retornados.
